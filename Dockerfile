...
# Copy the app
WORKDIR /app
COPY --from=node-to-build /app/dist/frontend/browser /usr/share/nginx/html

COPY --from=node-to-build /app/dist/frontend/browser/assets/default.conf /etc/nginx/conf.d/
